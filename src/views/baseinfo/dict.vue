    ...
    <data-grid url="/baseinfo/dict/list" dataName="listQuery" ref="dataList" @dataRest="onDataRest">
      <template slot="form">
        <el-form-item label="名称">
          <el-input v-model="listQuery.chnName" placeholder="名称" size="small" class="filter-item"
      ...
      <template slot="body">
        ...
        <el-table-column label="操作" align="center" width="170" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            ...
            <el-button type="danger" size="mini" icon="el-icon-delete" @click="dropRow(scope.row)"
                       title="删除"></el-button>
          </template>
        </el-table-column>
      </template>
    ...
    <el-dialog :visible.sync="modifyVisible">
      <el-tabs v-model="tabName" @tab-click="handleClick">
        <el-tab-pane label="字典详情" name="baseInfo">
          <el-form :model="dictForm" :rules="rules" ref="dictForm" label-width="70px" label-position="right"
                   size="small" style="width: 400px; margin-left:50px;">
        ...
        <el-tab-pane label="枚举值" name="dictMnt" :disabled="!isShowDictMnt">
          <el-form class="dialogTable" size="small">
            <div style="text-align:right;">
              <el-button type="primary" @click.native="appendEnum" size="mini"><i class="fa fa-plus-circle"
            ...
            <el-table :data="dictMntList.dictMnts" ref="multipleTable" stripe tooltip-effect="dark"
              ...
              <el-table-column prop="code" label="编码" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span v-if="scope.row.modifyFlag">
                    <el-input v-model="scope.row.code" type="text" placeholder="请输入..." style="width:100%"
                              size="small"></el-input>
                  </span>
                  <span v-else>
                    {{ scope.row.code }}
              ...
              <el-table-column prop="chnName" label="名称" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span v-if="scope.row.modifyFlag">
                    <el-input v-model="scope.row.chnName" type="text" placeholder="请输入..." style="width:100%"
                              size="small"></el-input>
                  </span>
                  <span v-else>
                    {{ scope.row.chnName }}
              ...
              <el-table-column prop="engName" label="英文" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span v-if="scope.row.modifyFlag">
                    <el-input v-model="scope.row.engName" type="text" placeholder="请输入..." style="width:100%"
                              size="small"></el-input>
                  </span>
                  <span v-else>
                    {{ scope.row.engName }}
              ...
              <el-table-column prop="showOrder" label="排序">
                <template slot-scope="scope">
                  <span v-if="scope.row.modifyFlag">
                    <el-input v-model="scope.row.showOrder" type="text" placeholder="请输入..." style="width:100%"
                              size="small"></el-input>
                  </span>
                  <span v-else>
                    {{ scope.row.showOrder }}
              ...
              <el-table-column label="操作" align="center" width="100" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                  <el-button type="danger" size="mini" icon="el-icon-delete"
                             @click="removeEnum(scope.$index, scope.row)" title="移除"></el-button>
                </template>
              </el-table-column>
            </el-table>
